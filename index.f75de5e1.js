...
function descF(e, i) {
    switch(e.target.textContent){
        ...
        case "Office":
            const newBodyRows1 = [
                ...bodyRows
            ].sort((a, b)=>b.cells[i].textContent.localeCompare(a.cells[i].textContent));
...
function ascF(e, i) {
    switch(e.target.textContent){
        ...
        case "Office":
            const newBodyRows1 = [
                ...bodyRows
            ].sort((a, b)=>a.cells[i].textContent.localeCompare(b.cells[i].textContent));
            bodyRows.forEach((node)=>node.parentNode.removeChild(node));
            newBodyRows1.forEach((node)=>tableBody.appendChild(node));
            break;
        case "Age":
...
form.addEventListener("submit", (e)=>{
    ...
    newEmployee.salary = `$${(newEmployee.salary / 1000).toFixed(3).toString().replace(".", ",")}`;
